...
describe('BaggageBuilderUtils', () => {
  ...
  it('should populate all baggage pairs from TurnContext', () => {
    ...
    expect(asObj[OpenTelemetryConstants.AGENT_USER_ID_KEY]).toBeUndefined();
    expect(asObj[OpenTelemetryConstants.GEN_AI_AGENT_DESCRIPTION_KEY]).toBe('agent');
    expect(asObj[OpenTelemetryConstants.AGENT_BLUEPRINT_ID_KEY]).toBeUndefined();
    expect(asObj[OpenTelemetryConstants.AGENT_EMAIL_KEY]).toBeUndefined();
