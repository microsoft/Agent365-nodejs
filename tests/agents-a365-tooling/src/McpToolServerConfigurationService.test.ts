...
describe('McpToolServerConfigurationService', () => {
  ...
  describe('listToolServers in development environment', () => {
    ...
    it('should read manifest from current working directory', async () => {
      ...
      expect(mockedFs.existsSync).toHaveBeenCalledWith(path.join('/test/project', 'ToolingManifest.json'));
      expect(mockedFs.readFileSync).toHaveBeenCalledWith(path.join('/test/project', 'ToolingManifest.json'), 'utf-8');
    ...
    it('should fallback to argv[1] location when manifest not found in cwd', async () => {
      ...
      expect(mockedFs.existsSync).toHaveBeenCalledWith(path.join('/test/project', 'ToolingManifest.json'));
      expect(mockedFs.existsSync).toHaveBeenCalledWith(path.join('/app/dist', 'ToolingManifest.json'));
